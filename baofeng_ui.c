...
void baofeng_scene_channel_list_on_enter(void* context) {
    ...
    for(uint8_t i = 0; i < BAOFENG_CHANNELS_COUNT; i++) {
        if(strlen(app->channel_names[i]) > 0) {
            snprintf(app->menu_names[i], 32, "CH %d: %s", i + 1, app->channel_names[i]);
        } else {
            snprintf(app->menu_names[i], 32, "CH %d", i + 1);
        }
        submenu_add_item(app->submenu, app->menu_names[i], i, baofeng_channel_list_callback, app);
    }
    view_dispatcher_switch_to_view(app->view_dispatcher, BaofengViewSubmenu);
}
...
static void freq_enter_callback(void* context, uint32_t index) {
    ...
    if (index == 0) { // RX Freq
        ...
        app->text_input_mode = 1;
        uint32_t freq_10hz = app->current_tx_freq_10hz;
        snprintf(app->text_store, sizeof(app->text_store), "%lu", freq_10hz);
        scene_manager_next_scene(app->scene_manager, BaofengSceneTextInput);
    } else if (index == 13) { // Comment
        scene_manager_next_scene(app->scene_manager, BaofengSceneCommentView);
    }
}
...
void baofeng_scene_channel_edit_on_enter(void* context) {
    ...
    // Comment
    app->item_comment = variable_item_list_add(app->variable_item_list, "Comment", 1, NULL, app);
    if(strlen(app->channel_names[app->current_edit_channel]) > 0) {
        variable_item_set_current_value_text(app->item_comment, "View/Edit");
    } else {
        variable_item_set_current_value_text(app->item_comment, "Empty");
    }
...
// --- Comment View Scene ---
static void baofeng_comment_view_callback(GuiButtonType result, InputType type, void* context) {
    BaofengApp* app = context;
    if(type == InputTypeShort && result == GuiButtonTypeRight) {
        view_dispatcher_send_custom_event(app->view_dispatcher, BaofengCustomEventCommentEdit);
    }
}

void baofeng_scene_comment_view_on_enter(void* context) {
    BaofengApp* app = context;
    widget_reset(app->widget);
    
    char* comment = app->channel_names[app->current_edit_channel];
    if(strlen(comment) == 0) {
        widget_add_text_scroll_element(app->widget, 0, 0, 128, 52, "No comment set for this channel.");
    } else {
        widget_add_text_scroll_element(app->widget, 0, 0, 128, 52, comment);
    }
    
    widget_add_button_element(app->widget, GuiButtonTypeRight, "Edit", baofeng_comment_view_callback, app);
    
    view_dispatcher_switch_to_view(app->view_dispatcher, BaofengViewWidget);
}

bool baofeng_scene_comment_view_on_event(void* context, SceneManagerEvent event) {
    BaofengApp* app = context;
    if(event.type == SceneManagerEventTypeCustom && event.event == BaofengCustomEventCommentEdit) {
        app->text_input_mode = 2; // Edit comment
        strncpy(app->text_store, app->channel_names[app->current_edit_channel], sizeof(app->text_store) - 1);
...
void baofeng_scene_comment_view_on_exit(void* context) {
    BaofengApp* app = context;
    widget_reset(app->widget);
}
...
static void text_input_callback(void* context) {
    ...
    if (app->text_input_mode == 2) {
        // Comment edit
        strncpy(app->channel_names[app->current_edit_channel], app->text_store, 199);
        app->channel_names[app->current_edit_channel][199] = '\0';
        scene_manager_previous_scene(app->scene_manager);
        return;
    }
...
void baofeng_scene_text_input_on_enter(void* context) {
    ...
    if (app->text_input_mode == 2) {
        text_input_set_header_text(app->text_input, "Channel Comment");
    } else {
...
void (*const baofeng_on_enter_handlers[])(void*) = {
    ...
    baofeng_scene_comment_view_on_enter,
...
bool (*const baofeng_on_event_handlers[])(void*, SceneManagerEvent) = {
    ...
    baofeng_scene_comment_view_on_event,
...
void (*const baofeng_on_exit_handlers[])(void*) = {
    ...
    baofeng_scene_comment_view_on_exit,
